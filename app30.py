...
if uploaded_file is not None:
    ...
    if len(target_counts) < 2:
        ...
        st.write(f"{selected_targets[0]} 样本数: {len(group1)}")
        st.write(f"{selected_targets[1]} 样本数: {len(group2)}")

        if len(group1) == 0 or len(group2) == 0:
            st.error("某个类别的样本数为0，无法进行对比。")
        else:
            ...
            for col in top_diff_columns.index:
                ...
                bin_count = 15 if len(group1) > 50 else 5 

                fig, ax = plt.subplots()
                ax.hist(group1[col], bins=bin_count, alpha=0.5, label=f'{selected_targets[0]}', color='#4c72b0', range=col_range, density=True)
                ax.hist(group2[col], bins=bin_count, alpha=0.5, label=f'{selected_targets[1]}', color='#dd8452', range=col_range, density=True)
